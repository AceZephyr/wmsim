...
def run(igt: int, start_hold: bool, menus: int):
    ...
    try:
        ...
        # snow
        for _ in range(140 + 2 * 17):
            s.walk(0xC, 0x0, True, chocotracks=False)

        for _ in range(10000):
            s.walk(0xC, 0x0, True, chocotracks=True)
...
def run_second(igt: int):
    ...
    for start_hold in [True, False]:
        for menus in range(4):
            ...
            if b.battle_id in {214, 215}:
                est_time = s.walkframes + (30 * menus) + (0 if start_hold else 60)
                if est_time < 450 and (best is None or best[5] > est_time):
...
with open("choco_mideel.txt", "w") as file:
    file.writelines(arr)
